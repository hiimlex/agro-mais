  ...
  <v-row class="my-auto" v-if="produtos" >
    ...
    <v-col  cols="6" xs="6" sm="4" md="4" lg="3" xl="3" class="d-flex" v-for="produto in produtos" :key="produto.id">
      <v-card
        ...
        elevation="2"
        link :to="{name: 'produto', params:{id: produto.id}}"
      > 
          <v-img
            max-height="60%"
            min-height="52%"
            :src="produto.imagem_produto[0].imagem"
          >
          </v-img>
        <v-card-title class="justify-center mb-n2 text-no-wrap">{{produto.nome}}</v-card-title>
        <v-card-text class="justify-center mb-n5">
          <v-row class="justify-center subtitle-1 text-truncate">{{produto.produtor.pessoa[0].nome}}</v-row>
